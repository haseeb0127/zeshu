...
# Point to your new live Vercel cloud!
...
for coord in path:
    try:
        # Sending the location to your Vercel backend
        response = httpx.post(f"{API_BASE_URL}/admin/update-location/1", json=coord)
        if response.status_code == 200:
            print(f"📍 Driver moved to: {coord['lat']}, {coord['lng']}")
        else:
            print(f"❌ Error sending location. Status Code: {response.status_code}")
