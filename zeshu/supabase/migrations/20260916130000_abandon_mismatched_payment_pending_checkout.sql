-- Abandon only an expired, provider-reconciled checkout whose basket no
-- longer matches the customer's current checkout.  The server must perform
-- live Razorpay reconciliation before invoking this service-role RPC.

create function public.abandon_mismatched_payment_pending_checkout(
...
begin
  ...
  -- Reuse the established ownership-safe reward-release implementation.
  return query
